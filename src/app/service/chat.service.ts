...
export class ChatService {
  ...
  /**
   * ✅ BACKWARD COMPAT signature (your component calls this with 11 args)
   * Emits to socketService.emit('chatMessage', payload)
   */
  async sendMessage(
    http: HttpClient,
    environmentRef: any,
    ...
    if (!ownerId || !selectedChat.buyerId || !productId) return;
    ...
    // ✅ join room before sending (important)
    socketService.emit('joinRoom', { chatId });
  ...
  fetchChatList(userId: string, token: string): Observable<ChatSummary[]> {
    ...
    return this.http.get<ChatSummary[]>(
      `${environment.apiBaseUrl}/api/chat/list/${userId}`,
      { headers }
    );
  }
